...
import java.sql.PreparedStatement;//statement클래스를 상속받음, 쿼리 먼저 보내고 리터럴 값은 나중에 보내는 역할
import java.sql.SQLException;
import java.sql.Statement;//작업지시 해서 데이터베이스 보내서 작업처리 한다음 받아오는 역할 
...
public class DatabaseTest {
	...
	public void insertDataFromFile() throws ClassNotFoundException, SQLException {
		...
		//PreparedStatement 클래스 만드는 과정, 메소드 안에 sql을 넣어서 만든다 
	...
	public Connection makeConnection() throws ClassNotFoundException, SQLException {
		...
		String url = "jdbc:oracle:thin:@localhost:1521:xe";//컴퓨터 위치
		String id = "HR";
		String pwd = "1234";
		
		Class.forName(driver);
		con = DriverManager.getConnection(url,id,pwd);
		//클래스를 오브젝트로 만들어서 로딩하는 역할, 위에 driver 객체에 접근해서 
		//위에 String driver 사이트가서 url,id,pw를 읽어옴
		return con;
	}
	...
	public void insertData() throws ClassNotFoundException, SQLException {
		...
		//Statement 클래스 만드는법 connection 객체에서 createstatement 객체를 참조해 만듬
